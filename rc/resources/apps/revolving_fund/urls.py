...
urlpatterns = patterns(
    ...
    url(r'^campus-sustainability-revolving-loan-funds/year/(?P<year>\d{4})/$',
        views.FundByYear.as_view(model=RevolvingLoanFund),
        name='revolving-fund-year'),
    url(r'^campus-sustainability-revolving-loan-funds/control/(?P<control>public|private)/$',
        views.FundTypeView.as_view(),
        name='revolving-fund-control'),
    url(r'^campus-sustainability-revolving-loan-funds/carnegie/(?P<carnegie>.+)/$',
        views.FundCarnegieView.as_view(),
        name='revolving-fund-carnegie'),    
    url(r'^campus-sustainability-revolving-loan-funds/1-million/$',
        views.FundListView.as_view(
    ...
    url(r'^campus-sustainability-revolving-loan-funds/region/(?P<region>[\w-]+)/$',
        views.FundByRegion.as_view(),
        name='revolving-fund-region'),
    url(r'^campus-sustainability-revolving-loan-funds/billion-dollar-green-challenge/$',
        views.FundListView.as_view(
            queryset=RevolvingLoanFund.objects.published().filter(
                billion_dollar=True),
            template_name='revolving_fund/revolvingloanfund_billion.html'),
        name='revolving-fund-billion'),    
    url(r'^campus-sustainability-revolving-loan-funds/state/(?P<state>[A-Z]+)/$',
