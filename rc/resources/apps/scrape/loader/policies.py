...
class PolicyLoader(BaseLoader):
    def __init__(self, parser_class, model_or_string, **kwargs):
        ...
        if self.kwargs.has_key('policy_type'):
            self.policy_type = PolicyType.objects.get(type=self.kwargs['policy_type'])
        else:
            raise AttributeError('PolicyLoader requires `policy_type` kwarg')
        if self.kwargs.has_key('resource_area'):
            self.resource_area = ResourceArea.objects.get(
